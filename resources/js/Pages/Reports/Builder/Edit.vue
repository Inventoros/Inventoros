...
import { ArrowLeft, Eye, Plus, Trash2, ChevronUp, ChevronDown, RefreshCw } from 'lucide-vue-next';
...
const form = useForm({
    ...
    data_source: props.report.data_source,
    columns: props.report.columns || [],
    filters: props.report.filters || [],
    sort: props.report.sort || { field: '', direction: 'asc' },
    chart_type: props.report.chart_type || null,
    chart_field: props.report.chart_field || null,
...
watch(() => form.data_source, (newVal, oldVal) => {
...
const updateReport = () => {
    ...
    form.transform(() => data).put(route('reports.builder.update', props.report.id));
};
...
<template>
    <Head :title="t('reportBuilder.editReport')" />

    <AppLayout>
        <template #header>
            <div class="flex items-center gap-2 text-xs">
                <Link :href="route('reports.index')" class="text-text-tertiary hover:text-text-primary">Workspace</Link>
                <span class="text-text-tertiary">/</span>
                <Link :href="route('reports.builder.index')" class="text-text-tertiary hover:text-text-primary">{{ t('reports.title') }}</Link>
                <span class="text-text-tertiary">/</span>
                <span class="font-medium text-text-primary">{{ t('reportBuilder.editReport') }}</span>
            </div>
        </template>

        <PageHeader :title="t('reportBuilder.editReport')" :description="report.name">
            <template #actions>
                <Button variant="secondary" size="sm" as="Link" :href="route('reports.builder.show', report.id)">
                    <Eye :size="14" />
        ...
        <div class="mx-auto mt-6 max-w-4xl space-y-4">
            ...
            <Card v-if="form.data_source" :padded="false">
                <div class="flex items-center justify-between px-5 pt-5">
                    ...
                    <div class="flex items-center gap-2">
                        <button type="button" @click="selectAllColumns" class="text-xs font-medium text-brand transition-colors hover:text-brand-hover">{{ t('reportBuilder.selectAll') }}</button>
                        <span class="text-text-tertiary">|</span>
                        <button type="button" @click="deselectAllColumns" class="text-xs font-medium text-brand transition-colors hover:text-brand-hover">{{ t('reportBuilder.deselectAll') }}</button>
                    </div>
                </div>
                <div class="p-5">
                    <div class="grid grid-cols-2 gap-3 md:grid-cols-3">
                        <label
                            ...
                            :key="col.key"
                            class="flex cursor-pointer items-center gap-2 rounded-lg p-2 transition-colors hover:bg-surface-overlay"
                        >
                            <input
                                type="checkbox"
                    ...
                    <div v-if="form.columns.length > 0" class="mt-4 border-t border-border-subtle pt-4">
                        ...
                        <div class="space-y-1">
                            <div
                                ...
                                :key="colKey"
                                class="flex items-center gap-2 rounded-lg border border-border-subtle bg-surface-canvas px-3 py-1.5"
                            >
                                ...
                                <button
                                    ...
                                    :disabled="index === 0"
                                    class="rounded-md p-1 text-text-tertiary transition-colors hover:bg-surface-overlay hover:text-text-primary disabled:opacity-30"
                                >
                                    <ChevronUp :size="14" />
                                </button>
                                <button
                ...
                <div class="p-5">
                    <div class="space-y-3">
                        <div
                            ...
                            :key="index"
                            class="flex flex-wrap items-center gap-3 rounded-lg border border-border-subtle bg-surface-canvas p-3"
                        >
                            <select
                    ...
                    <Button type="button" variant="ghost" size="sm" class="mt-3" @click="addFilter">
                        <Plus :size="14" />
            ...
            <Card v-if="form.data_source && form.columns.length > 0" :padded="false">
                <div class="flex items-center justify-between px-5 pt-5">
                    ...
                    <Button type="button" variant="default" size="sm" :loading="previewLoading" :disabled="previewLoading" @click="previewReport">
                ...
                <div class="flex items-center justify-between p-5">
                    ...
                    <div class="flex gap-3">
                        ...
                        <Button
                            ...
                            @click="updateReport"
                        >
                            {{ form.processing ? t('reportBuilder.saving') : t('reportBuilder.updateReport') }}
                        </Button>
                    </div>
                </div>
            </Card>
